...
default_prompt: "Use the asset-gen tools (see asset-gen/SKILL.md) to generate images, 3D models, or animated sprites for this game."
...
**FREE hubs FIRST, generate SECOND.** Try the free CC0/permissive hubs before paying to
generate — it's free, instant, and consistent. Generate only what must match a specific look the
hubs don't have. Record every asset (free or paid) in the README asset table with its **licence**
and **in-game size**. Tools live at `$HERMES_HOME/skills/game-builder/asset-gen/tools/`; run from
the project root and keep runtime-loaded outputs under `src/assets/`.

## Free hubs first — `tools/asset_search.py`

```bash
# CC0 PBR textures (ambientCG — NO key, works immediately)
python3 $HERMES_HOME/skills/game-builder/asset-gen/tools/asset_search.py texture --query "wood floor" --download -o src/assets/tex
# low-poly 3D models (Poly Pizza — free key in POLY_PIZZA_KEY, get it at poly.pizza/api)
python3 $HERMES_HOME/skills/game-builder/asset-gen/tools/asset_search.py model --query "car" --download -o src/assets/glb
```

Prints JSON with `license` + `attribution` per hit; `--download` fetches (textures arrive as a
full PBR set: Color/Normal/Roughness/AO). Record the licence in the README table; keep the
attribution string for CC-BY. Curated CC0 packs (no search API) — pull by URL for a coherent look:
**Kenney** (kenney.nl/assets, CC0 sprites/3D/UI/audio), **Quaternius** (CC0 low-poly 3D),
**OpenGameArt** (mixed licences → filter CC0/CC-BY, record attribution).

Load downloaded assets correctly (Babylon guide's build trap): `import ...?url` or `public/` —
never a raw `/src/...` string, it 404s in the build.

---

## Generate what's missing (paid — confirm the spend first)

The generators below (Gemini/Grok/Tripo3D) need their own API keys (`GOOGLE_API_KEY`,
`XAI_API_KEY`, `TRIPO3D_API_KEY`) — ask the owner if unset. You can also use your **native image
tool** (gpt-image via the gateway) for a quick sprite/background. Generate PNG images and GLB 3D
models from text prompts — every paid call costs real money.
...
```bash
python3 $HERMES_HOME/skills/game-builder/asset-gen/tools/asset_gen.py image \
...
Read `$HERMES_HOME/skills/game-builder/asset-gen/rembg.md`. Key rule: **never prompt for a "transparent background"** (the generator bakes a checkerboard) — prompt a solid color, then matte it out.
...
```bash
python3 $HERMES_HOME/skills/game-builder/asset-gen/tools/asset_gen.py glb  --image ref.png -o model.glb     # 30¢ default / 60¢ --quality hd
python3 $HERMES_HOME/skills/game-builder/asset-gen/tools/asset_gen.py rig  --image ref.png -o rigged.glb    # +25¢, biped only
python3 $HERMES_HOME/skills/game-builder/asset-gen/tools/asset_gen.py retarget --rigged rigged.glb \
...
- A timeout in `glb`/`rig`/`retarget` does **not** mean server failure. The task id is already saved in the `<output>.tripo.json` sidecar. **Do not resubmit — that double-charges.** Resume for free instead:
  ```bash
  python3 $HERMES_HOME/skills/game-builder/asset-gen/tools/asset_gen.py resume -o model.glb
  ```
...
```bash
_log=$(mktemp)
result=$(python3 $HERMES_HOME/skills/game-builder/asset-gen/tools/asset_gen.py image --prompt "..." -o p.png 2>"$_log") || tail -20 "$_log"
```
